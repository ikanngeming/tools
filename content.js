...
(function() {
	...
	window.fetch = async function(...args) {
		...
		try {
			...
			try {
				...
				if (ct.includes('application/json')) logEntry.responseBody = await clonedResponse.json();
				else if (ct.includes('text/')) logEntry.responseBody = await clonedResponse.text();
				else {
	...
	const consoleLog = [];
	const _LEVELS = ["log", "warn", "error", "info", "debug", "assert", "table", "dir",
		"trace", "group", "groupEnd", "groupCollapsed", "time", "timeEnd",
	...
	function createPanel() {
		...
		registerPlugin({
			...
			async fetchData() {
				...
				if (!scripts.length) {
					...
					for (let i = 0; i < scripts.length; i++) {
						...
						if (content && !content.startsWith('[')) {
							...
							out += perf ?
								`    Status: ⚠ CSP Blocked (Size: ${(perf.transferSize/1024).toFixed(1)}KB, Time: ${Math.round(perf.duration)}ms)\n` :
				...
				if (inlineScripts.length) {
					...
					inlineScripts.forEach((s, i) => {
						...
						out += `[${i+1}] ${c.split('\n').length} lines${s.type ? ` | Type: ${s.type}` : ''}\n`;
						if (c) out += `    ${c.substring(0, 80).replace(/\n/g, ' ')}${c.length > 80 ? '...' : ''}\n`;
					});
					out += `\n`;
				}
				...
				if (!links.length) {
					...
					for (let i = 0; i < links.length; i++) {
						...
						if (content && !content.startsWith('[')) {
							...
							out += perf ?
								`    Status: ⚠ CSP Blocked (Size: ${(perf.transferSize/1024).toFixed(1)}KB)\n` :
								`    Status: ✗ Failed\n`;
						}
			...
			async downloadAll() {
				...
				for (let i = 0; i < scripts.length; i++) {
					const s = scripts[i],
						content = await this.fetchContent(s.src, 'JS');
					scriptEntries.push({
						index: i + 1,
						type: 'external',
						src: s.src,
				...
				inlineScripts.forEach((s, i) => scriptEntries.push({
				...
				for (let i = 0; i < links.length; i++) {
					const l = links[i],
						content = await this.fetchContent(l.href, 'CSS');
					styleEntries.push({
						index: i + 1,
						type: 'external',
						href: l.href,
				...
				const exportData = {
					...
					summary: {
						...
						successRate: `${((fetchedCount/all.length)*100).toFixed(1)}%`
					},
